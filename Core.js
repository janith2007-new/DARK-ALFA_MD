...
module.exports = async (Miku, m, commands, chatUpdate, store) => {
    try {
            ...
            if (m.isGroup && csts == "true" && !icmd && !isCmd) {
                if (m.quoted) {
                    if (m.quoted.sender == botNumber) {
                        const botreply = await axios.get(`http://api.brainshop.ai/get?bid=173771&key=oP8Lu3VfDV9gKmTS&uid=[uid]&msg=[${budy}]`)
        ...
        if (body.startsWith(prefix) && !icmd) {
            let mikutext = `එවැනි විධානයක් වැඩසටහන්ගත කර නොමැත *${pushname}* help! ටයිප් කරන්න *${prefix}help* හෝ මගේ සම්පූර්ණ විධාන ලැයිස්තුව ලබා ගැනීමට පහත බොත්තම ඔබන්න!\n`;
        ...
        if (
            ...
            let data = [];
            if (cmd.alias) data.push(`*ƊƋГƘ ѦしẜƋ ⱮƊ :* ${cmd.alias.join(", ")}`);
